...
function Filter() {
  const searchParams = useSearchParams(); // return a read-only object
  ...
  function handleFilter(filter) {
    // creates a new editable query params object based on the current searchParams
    const params = new URLSearchParams(searchParams); // built-in Web API
...
function Button({ children, filter, handleFilter, activeFilter }) {
  return (
    <button
      className={`hover:bg-primary-700 px-5 py-2 hover:cursor-pointer ${filter === activeFilter ? "bg-primary-700 text-primary-50" : ""}`}
